...
namespace Server.Repository
{
    class RegularMemberRepository : IRepository<RegularMember>
    {
        ...
        public List<String> hasUsernamePassword(string username, string password)
        {
            ...
            using (IDbConnection db = new SqlConnection(ConfigurationManager.ConnectionStrings["cmsDatabase"].ConnectionString))
            {
                return res = db.Query<String>("SELECT CNP FROM RegularMembers WHERE Username='" + username + "' AND Passwd='" + password + "'").ToList();
            }
        }
        ...
        public void Update(String CNP, String firstName, String lastName, String affiliation)
        {
            using (IDbConnection db = new SqlConnection(ConfigurationManager.ConnectionStrings["cmsDatabase"].ConnectionString))
            {
                db.Query<String>("UPDATE RegularMembers SET FirstName='" + firstName + "', LastName='" + lastName + "', Affiliation='" + affiliation +"' WHERE CNP='" + CNP + "'").ToList();
            }
        }
